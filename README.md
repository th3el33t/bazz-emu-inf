...
- [x] Sunshine streaming (headless, virtual display): native RPM (lizardbyte/stable COPR), system service, first-boot credential bootstrap — Moonlight pairing is the owner-verification step. Capture is `capture = kwin` (KWin/PipeWire ScreenCast), **not** raw KMS: a `global_prep_cmd` hook (`~/.config/sunshine/hooks/`, the Sunveil pattern) spins up a `krfb-virtualmonitor` output at the connecting client's exact resolution, disables the physical output for the stream, and tears it down on disconnect. This is what makes streaming work with the Game Den OLED **off** — do NOT go back to KMS-capturing the physical panel: unlike Windows, the LG C9 on Linux/NVIDIA drops its DRM output the moment it sleeps (nvidia-drm honours no `edid_firmware`), so KMS capture dies with the TV. The unit carries the Wayland session env + a wait-for-KWin `ExecStartPre` (uinput button-cap race fix). Linux `sunshine.conf`/`apps.json` + hooks are seeded from `/usr/share/bazz-emu-inf/sunshine/` on first boot. **Auto-suspend is disabled image-wide** (`sleep.conf.d`): a headless box that suspends on idle falls off the network and looks exactly like a hard freeze — the HA Zigbee-plug watchdog is the recovery of last resort, not the design
...
- [x] Config + save restore: `tools/restore-backup.sh` (run from cc-homelab) pushes the migration backup into the data layer — ES-DE gamelists/settings + downloaded_media, RetroArch cfg/saves/BIOS, Steam userdata, Minecraft world. Sunshine config is **not** restored here (the image seeds the Linux version; the backup's copy is Windows-era and would clobber it)
- [ ] Cosign image signing — CI **does** sign every push keyless (Sigstore Fulcio + Rekor) and the trust anchors (`fulcio-intermediate-ca.crt.pem`, `rekor.pub`) ship in the image, but **box-side enforcement is deliberately OFF**: `policy.json` is `insecureAcceptAnything`, `use-sigstore-attachments: true` stays in `registries.d`. th3el33t's GitHub email-visibility is private, so the workflow's OIDC token has no `email` claim and the Fulcio cert carries no email SAN — `sigstoreSigned`/fulcio verification then rejects the box's **own** legitimately-signed image (`Required email not found`, containers/image `policy_eval_sigstore.go`). Re-enable by flipping `policy.json` back to `sigstoreSigned`+`fulcio` with the matching `subjectEmail` once email-visibility is made public (or cosign gains an email override); the staged anchors work as-is. See PR #27
